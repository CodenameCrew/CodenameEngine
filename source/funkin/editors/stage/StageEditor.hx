	...
	function buildStage():String {
		...
		for(sprite in getSprites()) {
			...
			if(button is StageSolidButton) {
				...
				saveToXml(spriteXML, "x", Math.round(sprite.x), 0);
				saveToXml(spriteXML, "y", Math.round(sprite.y), 0);
				...
				saveToXml(spriteXML, "skewx", FlxMath.roundDecimal(sprite.skew.x, 3), 0);
				saveToXml(spriteXML, "skewy", FlxMath.roundDecimal(sprite.skew.y, 3), 0);
				saveToXml(spriteXML, "alpha", FlxMath.roundDecimal(sprite.alpha, 3), 1);
				saveToXml(spriteXML, "angle", FlxMath.roundDecimal(sprite.angle, 3), 0);
				...
				saveToXml(spriteXML, "zoomfactor", FlxMath.roundDecimal(sprite.zoomFactor, 3), 1);
				...
				saveToXml(charXML, "x", Math.round(char.x), defaultPos.x);
				saveToXml(charXML, "y", Math.round(char.y), defaultPos.y);
				saveToXml(charXML, "camxoffset", Math.round(char.extra.get(exID("camX"))), 0);
				saveToXml(charXML, "camyoffset", Math.round(char.extra.get(exID("camY"))), 0);
				saveToXml(charXML, "skewx", FlxMath.roundDecimal(char.skew.x, 3), 0);
				saveToXml(charXML, "skewy", FlxMath.roundDecimal(char.skew.y, 3), 0);
				saveToXml(charXML, "spacingx", Math.round(char.extra.get(exID("spacingX"))), 20);
				saveToXml(charXML, "spacingy", Math.round(char.extra.get(exID("spacingY"))), 0);
				saveToXml(charXML, "alpha", FlxMath.roundDecimal(char.alpha / 0.75, 3), 1);
				saveToXml(charXML, "angle", FlxMath.roundDecimal(char.angle, 3), 0);
				saveToXml(charXML, "zoomfactor", FlxMath.roundDecimal(char.zoomFactor, 3), 1);
