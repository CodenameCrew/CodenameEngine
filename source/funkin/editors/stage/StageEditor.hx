	...
	function buildStage():String {
		...
		for(sprite in getSprites()) {
			...
			if(button is StageSolidButton) {
				...
				saveToXml(spriteXML, "x", FlxMath.roundDecimal(sprite.x, 0), 0);
				saveToXml(spriteXML, "y", FlxMath.roundDecimal(sprite.y, 0), 0);
				...
				saveToXml(spriteXML, "angle", FlxMath.roundDecimal(sprite.angle, 1), 0);
				...
				saveToXml(charXML, "x", FlxMath.roundDecimal(char.x, 0), defaultPos.x);
				saveToXml(charXML, "y", FlxMath.roundDecimal(char.y, 0), defaultPos.y);
				saveToXml(charXML, "camxoffset", FlxMath.roundDecimal(char.extra.get(exID("camX")), 0), 0);
				saveToXml(charXML, "camyoffset", FlxMath.roundDecimal(char.extra.get(exID("camY")), 0), 0);
				saveToXml(charXML, "skewx", FlxMath.roundDecimal(char.skew.x, 2), 0);
				saveToXml(charXML, "skewy", FlxMath.roundDecimal(char.skew.y, 2), 0);
				saveToXml(charXML, "spacingx", FlxMath.roundDecimal(char.extra.get(exID("spacingX")), 0), 20);
				saveToXml(charXML, "spacingy", FlxMath.roundDecimal(char.extra.get(exID("spacingY")), 0), 0);
				saveToXml(charXML, "alpha", FlxMath.roundDecimal(char.alpha / 0.75, 2), 1);
				saveToXml(charXML, "angle", FlxMath.roundDecimal(char.angle, 1), 0);
