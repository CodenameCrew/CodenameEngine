	...
	public function onKeyDown(e:KeyCode, modifier:KeyModifier) {
		switch (e) {
			...
			case END:
				position = label.text.length;
			case V:
				UIState.playEditorSound(modifier.ctrlKey ? Flags.DEFAULT_EDITOR_PASTE_SOUND : Flags.DEFAULT_EDITOR_TEXTTYPE_SOUND);
				...
				if (data != null)
					onTextInput(data);
			case C:
				UIState.playEditorSound(modifier.ctrlKey ? Flags.DEFAULT_EDITOR_COPY_SOUND : Flags.DEFAULT_EDITOR_TEXTTYPE_SOUND);
				// if we are not holding ctrl, ignore
				if (!modifier.ctrlKey)
					return;
			...
			case X:
				UIState.playEditorSound(modifier.ctrlKey ? Flags.DEFAULT_EDITOR_CUT_SOUND : Flags.DEFAULT_EDITOR_TEXTTYPE_SOUND);
